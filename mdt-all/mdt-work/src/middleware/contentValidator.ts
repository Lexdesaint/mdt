...
export const enhancedSqlInjectionDetector = (req: Request, res: Response, next: NextFunction) => {
  const sqlPatterns = [
    // Basic SQL keywords
  ...
  const checkObject = (obj: any, path: string = ''): string[] => {
    ...
    if (obj && typeof obj === 'object') {
      ...
      if (Object.prototype.hasOwnProperty.call(obj, key)) {
        ...
        const isWhitelisted = whitelistedFields.some(
          field =>
        ...
        if (typeof value === 'string' && !isWhitelisted && checkForSqlInjection(value)) {
          vulnerabilities.push(
            `Potential SQL injection detected in ${currentPath}: ${value.substring(0, 50)}...`
          );
